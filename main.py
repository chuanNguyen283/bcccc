...
TELEGRAM_BOT_TOKEN = os.environ.get(
    "TELEGRAM_BOT_TOKEN",
    "7940359228:AAHALTRYhvS4Rq_kvNlOrz5sbSVwroF_0ZE",
)
bot = telebot.TeleBot(TELEGRAM_BOT_TOKEN)
FONT_NAME = "Times New Roman"
FONT_SIZE = 11
def _to_naive_local(dt):
    """Chuyển đổi datetime sang múi giờ Việt Nam (UTC+7) và bỏ tzinfo."""
    if dt is None:
        return None
    tz_vn = timezone(timedelta(hours=7))
    if dt.tzinfo is None:
        # Nếu API trả về naive datetime, giả định nó là UTC
        dt = dt.replace(tzinfo=timezone.utc)
    return dt.astimezone(tz_vn).replace(tzinfo=None)
...
def in_week(created_at, week_start: datetime, week_end: datetime):
    ...
    if not dt:
        return False
    # Đảm bảo tất cả đều là naive (không múi giờ) trước khi so sánh
...
def tao_bao_cao_tuan():
    tz_vn = timezone(timedelta(hours=7))
    # Chuyển sang giờ VN rồi bỏ tzinfo để so sánh được với naive datetime từ _parse_dt
    now = datetime.now(tz_vn).replace(tzinfo=None)
    week_start, week_end = week_bounds(now)
    api_token = os.environ.get(
        "QUIK_API_TOKEN",
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjU2LCJlbWFpbCI6ImtoYW5odGRAdmluYXR0aS52biIsInJvbGUiOiJlbXBsb3llZSIsImlhdCI6MTc3Njc1MTkxNiwiZXhwIjoxNzc3MzU2NzE2fQ.YoT0fiPkC9w3568hCli3wcbhr3F8rJIQUGsZJXfCIyk",
    )
